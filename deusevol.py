...
class Beast(pygame.sprite.Sprite):
    def __init__(self, x, y, gmelanine, gskincrange, gskinbalance, \
                 female, gmuscularity, gspeed, ghairred):
        pygame.sprite.Sprite.__init__(self)
        self.married = False
        self.dead = False
        self.female = female
        ...
        if not self.female:
            ...
            self.hairback = pygame.transform.scale(self.hairback, (cellsize, cellsize))
        ...
        red, green, blue = 30, 0, 30
        red += int(self.ghairlight * 225)
        green = int((red/2) + (red/2) * (1 - self.ghairred))
        if self.hairback:
            ...
            self.image.blit(self.hairback, (0, 0))
    ...
    def sex_behave(self):
        if not self.female and not self.married:
            ...
            while len(prefered) > 1:
                if prefered[0].attract > prefered[1].attract:
...
player = Player()
print(beast_coords[0][0], beast_coords[0][1])
